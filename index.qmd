...
- El muestreo aleatorio simple (MAS) consiste en seleccionar unidades al azar desde un listado completo de la población.
    ...
    [<img src="imagenes/plots/MAS.jpeg" width="40%"/>]{.center}
...
<!-- “El Marco Muestral de Viviendas (MMV)”  ->  “Un marco muestral maestro es una estructura metodológica diseñada para cubrir completamente el territorio nacional mediante unidades geográficas organizadas, que permiten seleccionar muestras probabilísticas a lo largo del período intercensal. De acuerdo con las recomendaciones de Naciones Unidas (2009), se concibe como una base común, estable y coherente para múltiples encuestas o rondas repetidas de una misma operación estadística, garantizando trazabilidad y consistencia temporal”  -->
...
<!-- - Esta encuesta posee una representatividad a nivel regional -->
...
- En un diseño estratificado bietápico, la probabilidad de selección se construye como la multiplicación de probabilidades
    $$\pi_i = \pi_\text{upm}* \pi_\text{vivienda|upm}$$
...
- Supongamos que en cada UPM seleccionada tenemos 200 viviendas, y seleccionamos 60:
    $$\pi_\text{vivienda|upm}= 30/200 = 0.6$$
- Así, el factor de expansión teórico nos queda como: 
    $$\text{Factor expansión} = 1/(0.1*0.3) = 300 $$
...
```{r echo = TRUE}
enusc_2023 %>% head() %>% kableExtra::kbl()
```
...
dc <- survey::svydesign(ids = ~Conglomerado,
